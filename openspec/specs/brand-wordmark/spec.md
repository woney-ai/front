...
can express a sub-floor serif instance. `logo.tsx`, `single-use-card.tsx`,
and `agent-session.tsx` MUST route through it; no inline `font-display`
wordmark markup may remain at any call site.

#### Scenario: Header and card wordmarks meet the floor; sub-floor is unreachable
- GIVEN `logo.tsx` renders `variant="header"` and `single-use-card.tsx`
...
- GIVEN `agent-session.tsx` renders `variant="monogram"`
